...
static int do_dummy(unsigned int dummylen)
{
    DBPOD_CMDBUF_DUMMY cmd;

    memset(&cmd, 0, sizeof(cmd));
    cmd.hdr.dwLength = sizeof(cmd) - sizeof(ULONG);
    cmd.hdr.dwSequence = cmdseq++;
    cmd.hdr.wCmd = DBPOD_CMDCODE_DUMMY;
    cmd.dwReplyLen = dummylen;
    return do_cmd(&cmd.hdr);
}

static int do_test(unsigned dummylen, unsigned iter)
{
    unsigned int n;
    ...
    /*
     * Do an initial dummy message before the timings, because the pod
     * takes a short while to get the hardware ready.
     */
    do_dummy(0);
    ...
    secs = (end.tv_sec - start.tv_sec) + ((end.tv_nsec - start.tv_nsec) / 1e9);
    printf("%u msgs x %u B: time(s):%f; msg/s:%f; s/msg:%f; Mib/s:%f\n",
            iter, dummylen, secs, iter / secs, secs / iter,
            (double)iter * dummylen * 8 * 1e-6 / secs);
}
...
int main(int argc, char *argv[])
{
    ...
    unsigned int dummylen, iterations;
    static const int opttrue = 1;
    int err;

    progname = argv[0];
    if (argc < 5)
    {
        fprintf(stderr, "usage: %s HOST PORT DUMMYLEN ITERATIONS\n", progname);
        exit(2);
    }
    node = argv[1];
    service = argv[2];
    dummylen = strtoul(argv[3], NULL, 10);
    ...
    setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, &opttrue, sizeof(opttrue));
    err = do_test(dummylen, iterations);
    close(sock);
    if (err < 0)
    {
        exit(EXIT_FAILURE);

#ifndef PAUTPOD_COMS_H__INCLUDED
#define PAUTPOD_COMS_H__INCLUDED
...
/*
 * A phased-array TX element or RX element.
 *
 * For phased array, nTx in the channel config is set to the number of TX
 * elements being fired minus 32768 (0x8000), and nRx is set to the number
 * of RX elements minus 32768 (0x8000).
 *
 * The array of DBPOD_GATECFG for the gates in the channel config is
 * immediately followed by an array of DBPOD_ELEMCFG for the phased-array
 * TX elements, followed by an array of DBPOD_ELEMCFG RX elements.
 * The elements in each array may appear in any order.
 */
typedef struct TAG_DBPOD_ELEMCFG
{
    USHORT nIndex;                  /* Element (RX or TX) index. */
    SHORT pad;                      /* (padding) */
    ULONG dwTimeOffset;             /* Element time offset in nanoseconds. */
} DBPOD_ELEMCFG;

/* 'Channel/Sequence Configuration' command buffer (portion before the gates). */
/* (Note: db-Pod only supports nRepeat==1; PAUT-Pod supports nRepeat>0. */
/* For phased-array, the TX and RX elements appear after the gate configs. */
typedef struct TAG_DBPOD_CMDBUF_CHAN_CONFIG
{
    ...
    SHORT       nTx;                /* Physical connector to pulse,
                                     * or number of PA TX elements minus 32768,
                                     * or -1=do not pulse. */
    SHORT       nRx;                /* Physical receiver input channel,
                                     * or number of PA RX elements minus 32768,
                                     * or -1=disable receiver. */
    SHORT       nVoltage;           /* Pulser voltage in volts. */
    SHORT       nPulseWidth;        /* Either Pulse Width in nanoseconds,
                                     * or Pulse Shape Index minus 32768. */
...
/* If nTx < -1, followed by array DBPOD_ELEMCFG[nTx + 32768] of TX elements. */
/* If nRx < -1, followed by array DBPOD_ELEMCFG[nRx + 32768] of RX elements. */
...
/********************************************
 *
 * 'Channel/Sequence Clear' command and response.
 *
 ********************************************/
#define DBPOD_CMDCODE_CHAN_CLEAR           0x0035
#define DBPOD_RSPCODE_CHAN_CLEAR           0x8035

/* 'Channel/Sequence Clear' command buffer. */
typedef struct TAG_DBPOD_CMDBUF_CHAN_CLEAR {
    DBPOD_MSGHDR hdr;               /* Message header. */
    USHORT      nStartIndex;        /* First sequence table index to clear. */
    USHORT      nEndIndex;          /* Last sequence table index to clear. */
} DBPOD_CMDBUF_CHAN_CLEAR;

/* 'Channel/Sequence Clear' response buffer is just a message header. */
typedef struct TAG_DBPOD_RSPBUF_CHAN_CLEAR
{
    DBPOD_MSGHDR hdr;               /* Message header. */
} DBPOD_RSPBUF_CHAN_CLEAR;

/********************************************
 *
 * 'Pulse Shape Config' command and response.
 *
 ********************************************/
#define DBPOD_CMDCODE_PULSE_CONFIG         0x0036
#define DBPOD_RSPCODE_PULSE_CONFIG         0x8036

/*
 * 'Pulse Shape Config' command buffer.
 *
 * This sets a pattern of fire system element data (FSEDs) to be played out
 * over transmit pulsers at one of a number of supported frequencies.  The
 * same pattern is played out over each pulser at the same rate, but starting
 * at different time offsets.
 *
 * Each FSED is a 2-bit value indicating whether the pulser output should be
 * off (00), positive (01) or negative (10).
 *
 * Four FSEDs are packed into a byte:
 *
 *  Bit In Byte | 7 6 5 4 3 2 1 0
 * -------------+----------------
 * FSED In Byte | 3 3 2 2 1 1 0 0
 *    Direction | N P N P N P N P
 *
 * The FSEDs are played out starting at the first FSED (bits 1-0) in the
 * first byte.
 *
 * There can be multiple pulse shapes stored in the system.  Each one
 * has an index number.  The 'Soft Reset' command will clear all stored pulse
 * shapes.  The 'Pulse Shape Clear' command will clear a range of stored
 * pulse shapes.
 *
 * XXX Only pulse shape indices up to 32767 can be specified in the channel
 * configuration.
 */
typedef struct TAG_DBPOD_CMDBUF_PULSE_CONFIG {
    DBPOD_MSGHDR hdr;               /* Message header. */
    USHORT      nIndex;             /* Pulse shape index number. */
    USHORT      nLength;            /* Pulse shape length in 2-bit FSEDs. */
    UCHAR       bData[DBPOD_ANYLENGTH];    /* Pulse FSEDs (4 per byte). */
} DBPOD_CMDBUF_PULSE_CONFIG;
/* Basic size of DBPOD_CMDBUF_PULSE_CONFIG without bData[]. */
#define BASE_SIZE_DBPOD_CMDBUF_PULSE_CONFIG \
    FIELD_OFFSET(DBPOD_CMDBUF_PULSE_CONFIG, bData[0])
/* Followed by array UCHAR bData[(nLength+3)/4] pulse shape data. */

/* 'Pulse Shape Config' response buffer is just a message header. */
typedef struct TAG_DBPOD_RSPBUF_PULSE_CONFIG {
    DBPOD_MSGHDR hdr;               /* Message header. */
} DBPOD_RSPBUF_PULSE_CONFIG;

/********************************************
 *
 * 'Pulse Shape Clear' command and response.
 *
 ********************************************/
#define DBPOD_CMDCODE_PULSE_CLEAR          0x0037
#define DBPOD_RSPCODE_PULSE_CLEAR          0x8037

/* 'Pulse Shape Clear' command buffer. */
typedef struct TAG_DBPOD_CMDBUF_PULSE_CLEAR {
    DBPOD_MSGHDR hdr;               /* Message header. */
    USHORT      nStartIndex;        /* First pulse shape index to clear. */
    USHORT      nEndIndex;          /* Last pulse shape index to clear. */
} DBPOD_CMDBUF_PULSE_CLEAR;

/* 'Pulse Shape Clear' response buffer is just a message header. */
typedef struct TAG_DBPOD_RSPBUF_PULSE_CLEAR
{
    DBPOD_MSGHDR hdr;               /* Message header. */
} DBPOD_RSPBUF_PULSE_CLEAR;

...
static void dump_msg(const DBPOD_MSGHDR *hdr, int direction)
{
    ...
    if (hdr->dwLength < sizeof(DBPOD_MSGHDR) - sizeof(ULONG)
            || (hdr->dwLength & (sizeof(ULONG) - 1))
            || (hdr->dwLength == (ULONG)(0 - sizeof(ULONG))))
    {
    ...
    case DBPOD_CMDCODE_SCAN_CONFIG:
        msgstr = "SCAN_CONFIG";
        break;
    case DBPOD_CMDCODE_PA_CHAN_ELEM_DELAY:
        msgstr = "PA_CHAN_ELEM_DELAY";
        break;
...
static int expect_reply(USHORT msgcode, ULONG msgseq)
{
    ...
    while (ret == -1)
    {
        ...
        {
            /*
             * Set return value to -2 if message contains an error code,
             * else set return value to 0.
             */
            ret = msghdr->wSubCode ? -2 : 0;
        }
        free(msghdr);
    }
    return ret;
...
static int do_pa_balance_configs(void)
{
    int ret;
    struct {
        DBPOD_CMDBUF_PA_CHAN_ELEM_DELAY cmd;
        LONG xtravals[256];
    } m;

    memset(&m, 0, sizeof(m));
    m.cmd.hdr.wCmd = DBPOD_CMDCODE_PA_CHAN_ELEM_DELAY;
    m.cmd.nIndex = -1;
    ...
    m.cmd.bTypeMask = DBPOD_PAMASK_BAL_DELAY;
    m.cmd.hdr.dwSequence = cmdseq++;
    ret = do_cmd(&m.cmd.hdr);
    return ret;
}

static int do_pa_channel_configs(void)
{
    int ret;
    struct {
        DBPOD_CMDBUF_PA_CHAN_ELEM_DELAY cmd;
    ...
    m.cmd.hdr.wCmd = DBPOD_CMDCODE_PA_CHAN_ELEM_DELAY;
    m.cmd.nIndex = -1;
    m.cmd.bTypeMask = DBPOD_PAMASK_TX_ELEM | DBPOD_PAMASK_RX_ELEM;
    m.cmd.nCount = 16;
    for (e = 0; e < m.cmd.nCount; e++)
    {
        m.cmd.anValue[e] = e ^ 6;
    }
    m.cmd.hdr.dwLength =
    ...
    m.cmd.bTypeMask = DBPOD_PAMASK_TX_DELAY | DBPOD_PAMASK_RX_DELAY;
    for (e = 0; e < m.cmd.nCount; e++)
    {
        m.cmd.anValue[e] = e * 20;
    }
    m.cmd.hdr.dwSequence = cmdseq++;
    ret = do_cmd(&m.cmd.hdr);
    if (ret)
    ...
    m.cmd.bTypeMask = DBPOD_PAMASK_RX_GAIN;
    for (e = 0; e < m.cmd.nCount; e++)
    {
        m.cmd.anValue[e] = -(LONG)(e * 50);
    }
    m.cmd.hdr.dwSequence = cmdseq++;
    ret = do_cmd(&m.cmd.hdr);
    if (ret)
...
static int do_pa_configs(void)
{
    int ret;

    ret = do_pa_balance_configs();
    if (ret == 0)
    {
        ret = do_pa_channel_configs();
    }
    return ret;
}

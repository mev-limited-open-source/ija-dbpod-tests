...
#if 0
        if (n >= 32) {
            m.cmd.Gate[0].lWidth = 4000;
            m.cmd.Gate[1].lWidth = 4000;
        }
#endif
...
static int do_stop_ut(void)
{
    DBPOD_CMDBUF_STOP_UT cmd;

    memset(&cmd, 0, sizeof(cmd));
    cmd.hdr.dwLength = sizeof(cmd) - sizeof(ULONG);
    cmd.hdr.dwSequence = cmdseq++;
    cmd.hdr.wCmd = DBPOD_CMDCODE_STOP_UT;
    return do_cmd(&cmd.hdr);
}

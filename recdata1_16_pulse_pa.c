...
static int do_pulse_configs(void)
{
    int err;

    err = do_pulse_config(0, 10);
    return err;
}
...
static int do_chan_configs(void)
{
    struct {
        ...
        UCHAR xtrastuff[5 * sizeof(DBPOD_GATECFG) + 64 * sizeof(DBPOD_ELEMCFG)];
    } m;
    DBPOD_ELEMCFG *el;
    enum { N_TX_ELEMS = 2, N_RX_ELEMS = 3 };
    int i;
    ...
    m.cmd.dwDelay = 10000;
    m.cmd.nRepeat = 1;
    m.cmd.nTx = N_TX_ELEMS - 32768;
    m.cmd.nRx = N_RX_ELEMS - 32768;
    m.cmd.nDigBits = 16;    /* ! */
    m.cmd.nVoltage = 50;
    m.cmd.nPulseWidth = 0 - 32768;  /* Pulse shape 0 */
    m.cmd.nGain = 0;
    m.cmd.nHPF = 0;
    m.cmd.nLPF = 0;
    ...
    el = (DBPOD_ELEMCFG *)(void *)&m.cmd.Gate[2];
    /* TX elements */
    for (i = 0; i < N_TX_ELEMS; i++)
    {
        el->nIndex = i;
        el->dwTimeOffset = 0;
        el++;
    ...
    m.cmd.hdr.dwSequence = cmdseq++;
    m.cmd.hdr.dwLength = (char *)el - (char *)&m.cmd.hdr - sizeof(ULONG);
    return do_cmd(&m.cmd.hdr);
}
